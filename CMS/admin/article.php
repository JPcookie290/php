...
// If Abfrage ob ID vorhanden ist
if ( $id ) {
...
if ( $_SERVER['REQUEST_METHOD'] === 'POST' ) {
    ...
    if ( isset( $_FILES['filename'] ) ) {
        ...
        if ( $tmp_path && $image['error'] === UPLOAD_ERR_OK ) {
            ...
            $errors['filename'] .= in_array( $extension, $allowed_ext ) ? '' : 'The file extention is not allowed';
            // Bildgröße wird validiert
            $errors['filename'] .= $image['size'] > $max_size ? 'The images exceeds the maximum upload size' : '';
    ...
    // Error-Meldungen erstellen und zusätzliche Validierung
    $errors['title'] = is_text( $article['title'] ) ? '' : 'Title must be between 1 and 100 characters';
    $errors['summary'] = is_text( $article['summary'], 1, 200 ) ? '' : 'Summary must be between 1 and 100 characters';
    $errors['content'] = is_text( $article['content'], 1, 10000 ) ? '' : 'Content must be between 1 and 100 characters';
    ...
    if ( ! $problems ) {
        // bindings beinhaltet all Variablen die der Funktion Statemaent::bindValue übergeben werden (oder auch Statement::execute als Array)
        $bindings = $article;
        try {
            ...
            if ( $save_to ) {
                ...
                // lastInsertId gibt die ID des zuletzt einfügten Datensatzes zurück
            ...
            // Block Artikel wird bearbeitet
            if ( $id ) {
                // Wenn eine ID vorhanden ist, wird ein Update durchgeführt und die ID wird wieder in das bindings Array aufgenommen.
                $bindings['id'] = $id;
            ...
            // Wenn ein Fehler auftritt, wird die Transaktion zurückgerollr und der Fehler ausgegeben
            $pdo->rollBack();
