...
if ($id) {
    ...
    // Wenn die Kategorie nicht gefunden wurde, wird der Benutzer zu Auflistung aller Kategorien umgeleitet
...
if ( $_SERVER['REQUEST_METHOD'] === 'POST' ) {
    ...
    $category['navigation'] = filter_input( INPUT_POST, 'navigation' ) ?? 0;
    ...
    // Fehler werden in eine Zeichenkette zusammen gefasst
    ...
    if ( ! $problems ) {
        // Wenn die ID vorhanden ist, wird die Kategorie aktualisiert (UPDATE), ansonsten wird eine neue Kategoire in der Datenbank erstellt
        ...
        // Die zu speichernden Staen werden in ein Array zusammengefasst um später die Platzhalter zu ersetzten
        $bindings = [
            'name' => $category['name'],
        ...
        // Die Daten werden in die Datenbank gespeichert un der BEnutzer wird zur Kategorie-Liste umgeleitet
        try {
...
<main class="container w-auto mx-auto md:w-1/2 flex justify-center flex-col imtems-center p-5">
    <form class="w-full grid" action="category.php?id=<?= $id ?? '' ?>" method="post">
        ...
        <div class="p-4">
            ...
            type="text" name="name" id="name" value="<?= e( $category['name'] ) ?>">
        ...
        <div class="p-4">
            ...
            type="checkbox" name="navigation" id="navigation" <?= $category['navigation'] ? 'checked' : '' ?>>
